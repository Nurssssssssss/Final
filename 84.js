...
if (readMoreBtn && mainParagraph) {
  let isExpanded = false;
  const fullText = mainParagraph.innerHTML;
  const shortText = fullText.slice(0, 400) + "...";

  mainParagraph.innerHTML = shortText;
  ...
  readMoreBtn.addEventListener("click", () => {
    if (isExpanded) {
      ...
      mainParagraph.innerHTML = fullText;
...
if (searchCards && cardsContainer) {
  searchCards.addEventListener("keyup", () => {
    let filter = searchCards.value.toLowerCase();
